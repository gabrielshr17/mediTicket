...
export function createEmailWorker(): Worker<BookingEmailJob> {
...
export function startEmailWorkerIfEnabled(): Worker<BookingEmailJob> | null {
  if (process.env.RUN_WORKER_IN_WEB !== "true") return null;
  if (!process.env.REDIS_URL) {
    console.warn("[worker] RUN_WORKER_IN_WEB is set but REDIS_URL is not; worker not started");
    return null;
  }
